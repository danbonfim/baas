  ...
  const filtered = useMemo<Professional[]>(() => {
    ...
    if (query) {
      ...
      results = results.filter((p) =>
        p.name.toLowerCase().includes(q) || p.city.toLowerCase().includes(q) || p.tagline.toLowerCase().includes(q)
      )
    }
      ...
      <div className="sticky top-16 z-40 glass border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Sheet>
              <SheetTrigger className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium h-9 px-4 py-2 border border-white/10 bg-background hover:bg-accent hover:text-accent-foreground cursor-pointer">
                  ...
                  {viewMode === 'grid' ? (
                    <div className="group relative rounded-2xl overflow-hidden glass hover:border-brand-500/30 transition-all cursor-pointer">
                      ...
                      <div className="p-3 flex items-center justify-between">
                        ...
                        <div className="flex gap-1">
                          {p.categories.slice(0, 1).map((c) => <Badge key={c} variant="secondary" className="text-[10px] bg-white/5">{c}</Badge>)}
                        </div>
                      </div>
                    </div>
                  ) : (
