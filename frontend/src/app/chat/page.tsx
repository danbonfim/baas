...
import { Search, Send, ArrowLeft, Loader2, Wifi, WifiOff, MessageCircle, User } from 'lucide-react'
  ...
  return (
    <button
      ...
      className={`w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-white/5 transition-colors border-b border-white/5 ${
        isActive ? 'bg-brand-500/10 border-l-2 border-l-brand-500' : ''
      }`}
    >
      <div className="relative flex-shrink-0">
        ...
        {unread > 0 && (
          <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full gradient-brand text-white text-[10px] flex items-center justify-center">
            {unread}
          </span>
        )}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground truncate">
          {lastMsg
            ? (lastMsg.senderId === myId ? `Você: ${lastMsg.content}` : lastMsg.content)
...
export default function ChatPage() {
  ...
  return (
    <div className="h-[calc(100vh-64px)] flex overflow-hidden">
      ...
      {/* ── SIDEBAR ── */}
      <motion.div
        className={`${showSidebar ? 'flex' : 'hidden'} md:flex flex-col w-full md:w-80 lg:w-96 border-r border-white/10 bg-background/50 flex-shrink-0`}
        initial={false}
      >
        {/* Sidebar Header */}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
              ...
              <p className="text-xs text-muted-foreground">
                {search ? 'Tente outro nome' : 'Explore perfis e inicie uma conversa'}
              </p>
            </div>
          ) : (
            <div>
        ...
        {activeConversation ? (
          <>
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 glass">
              <Button
                ...
                onClick={() => { setShowSidebar(true); setActiveConversation(null) }}
              >
            ...
            <div className="p-4 border-t border-white/10 glass">
              <div className="flex items-center gap-3">
                <Input
                  ...
                  className="flex-1 bg-white/5 border-white/10 h-10"
                  disabled={sending}
                />
                <Button
                  onClick={handleSend}
                  disabled={!newMessage.trim() || sending}
                  className="gradient-brand text-white h-10 w-10 p-0 flex-shrink-0"
                >
          ...
          <div className="flex flex-col items-center justify-center flex-1 text-center px-6">
            ...
            <p className="text-muted-foreground text-sm max-w-xs mb-6">
              Selecione uma conversa ao lado ou explore perfis para iniciar uma nova
            </p>
            <Button
              className="md:hidden gradient-brand text-white gap-2"
