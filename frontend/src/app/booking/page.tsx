  ...
  return (
    <div className="min-h-screen pb-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        <Link href={slugParam ? `/profile/${slugParam}` : '/search'}>
          ...
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                step >= s ? 'gradient-brand text-white' : 'bg-white/5 text-muted-foreground'
              }`}>
              ...
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                <div className="glass rounded-2xl p-6">
                  <h2 className="font-bold text-lg mb-4 flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-brand-400" /> Selecione a data
                  </h2>
                    ...
                    {dates.map((date, i) => (
                      <button
                        ...
                        className={`p-3 rounded-xl text-center transition-all ${
                          selectedDate === i ? 'gradient-brand text-white' : 'bg-white/5 hover:bg-white/10'
                        }`}
                      >
                ...
                <div className="glass rounded-2xl p-6">
                  <h2 className="font-bold text-lg mb-4 flex items-center gap-2">
                    <Clock className="w-5 h-5 text-brand-400" /> Selecione o horário
                  </h2>
                    ...
                    {timeSlots.map((time) => (
                      <button
                        ...
                        className={`p-3 rounded-xl text-center text-sm transition-all ${
                          selectedTime === time ? 'gradient-brand text-white' : 'bg-white/5 hover:bg-white/10'
                        }`}
                      >
                        {time}
                      </button>
                    ...
                    {durations.map((d) => (
                      <button
                        ...
                        className={`p-4 rounded-xl text-center transition-all ${
                          selectedDuration === d.value ? 'gradient-brand text-white' : 'bg-white/5 hover:bg-white/10'
                        }`}
                      >
                ...
                <Button
                  className="w-full gradient-brand text-white h-12"
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          placeholder="Restaurante, hotel, endereço..."
            ...
            {step === 3 && (
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                {confirmed ? (
                  ...
                  <div className="glass rounded-2xl p-6">
                    <h2 className="font-bold text-lg mb-4 flex items-center gap-2">
                      <CreditCard className="w-5 h-5 text-brand-400" /> Pagamento
                    </h2>
                    <div className="space-y-4">
                      <div className="p-4 rounded-xl bg-brand-500/10 flex items-center gap-3">
                        ...
                        <p className="text-sm text-brand-300">
                          Pagamento seguro processado via Stripe. O valor só é debitado após confirmação.
                        </p>
                      </div>
                        ...
                        <div className="space-y-4">
                          <div className="p-4 rounded-xl bg-white/5 text-center">
                            ...
                            <p className="text-sm text-brand-400">
                              {clientSecret ? '✓ Payment Intent criado com sucesso' : 'Configure STRIPE_PUBLISHABLE_KEY para ativar'}
                            </p>
                          </div>
                          ...
                          <div className="flex gap-3">
                            ...
                            <Button
                              className="flex-1 gradient-brand text-white h-12"
          ...
          <div className="space-y-4">
            <div className="glass rounded-2xl p-5 sticky top-24">
              <div className="flex items-center gap-4 mb-4">
                <img
                  src={Array.isArray(professional.photos) && professional.photos[0] ? professional.photos[0] : `https://picsum.photos/seed/${professional.id}/200/200`}
              ...
              <div className="space-y-3 text-sm">
                {selectedDate !== null && (
                  <div className="flex justify-between">
                    ...
                    <span>{dates[selectedDate].toLocaleDateString('pt-BR')}</span>
                  </div>
                )}
                {selectedTime && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Horário</span>
                    <span>{selectedTime} – {addHours(selectedTime, selectedDuration)}</span>
                  </div>
                )}
                <div className="flex justify-between">
                ...
                <div className="flex justify-between text-lg font-bold">
                  <span>Total</span>
                  <span className="text-brand-400">R$ {(totalAmount + platformFee).toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
